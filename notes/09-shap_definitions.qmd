...
we can substitute it into @eq-shapley to get a local feature
...
1. To understand this definition geometrically, consider the figure below, which
represents $C(2 | 1)$ under marginal removal.
   ...
   - $C(1 \mid 2)$ flips the roles of the two coordinates. Now we replace the first coordinate with training values $x_{i1}$ rather than the second, so the dashed lines would run horizontally (in the $x_1$ direction) instead of vertically, and the cross-section would be taken along $x_2 = x_2$ rather than $x_1 = x_1$.
   - $C(1 \mid \emptyset)$ involves $v(\{1\}) - v(\emptyset)$. We already have $v(\{1\})$, it is the mean of the blue histogram above. For $v(\emptyset)$, we remove both coordinates, meaning we evaluate $f$ at the original training points without moving them to any dashed line. That is:

   $$
   v(\emptyset) = \frac{1}{N} \sum_{i} f(x_{i1}, x_{i2})
   $$

   - $\phi_1$ **vs.** $\phi_2$. We would expect $\phi_2 > \phi_1$ because the contours vary more steeply in the $x_2$ direction. Intuitively, the second coordinate matters more for explaining almost any prediction. More concretely, the analog of the right-hand panel for $C(1 \mid 2)$ would have a flatter cross-section, leading to a smaller gap between $f(x)$ and the histogram mean.
