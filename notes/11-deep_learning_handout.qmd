...
_Readings: [1](https://visionbook.mit.edu/neural_nets.html) (sections 12.1 - 12.7, but skip 12.5.2 - 12.5.3), [2](https://docs.pytorch.org/tutorials/beginner/basics/intro.html#learn-the-basics) (optional)_, _[Code](https://github.com/krisrs1128/stat479_notes/blob/master/notes/11-deep_learning_handout.qmd)_
...
|---|---|---| --- | --- |
| Image Classification | image | label | ![Task from @helberetal.](figures/imgage_classification.png){width=200px} |
| Object Detection | image | label + bounding box | ![Credit to Yagnik B on [Wikimedia](https://commons.wikimedia.org/wiki/File:YOLOv5_and_SAHI_interface_(Ariel_top_view_of_Ahmedabad,_Gujarat,_India,_2022).jpg).](figures/object_detection.jpg){width=200px} |
| Language Modeling | sequence of words | next word | ![](figures/next_word_pred.png){width=200px} |
| Sentence Translation | sequence of words | sequence of words | ![](figures/sentence_translation.png){width=200px}  |
| Style Transfer | image | new image | ![Mona Lisa in Starry Night style, from [Wikimedia](https://commons.wikimedia.org/wiki/File:Mona_lisa_the_starry_night_o_lbfgs_i_content_h_720_m_vgg19_cw_100000.0_sw_30000.0_tv_1.0.jpg).](figures/Mona_lisa_the_starry_night_o_lbfgs_i_content_h_720_m_vgg19_cw_100000.0_sw_30000.0_tv_1.0.jpg){width=200px} |
...
1. This works across many problem contexts. E.g., class labels $y_i \in \{1,
\dots, K\}$ become one-hot encoded vectors, so tensors capture standard
statistics problems. RGB images are 3D tensors (Height $\times$ Width $\times$
Color Channel), and video adds a Time axis, so their tensors are 4D. A document
is a Vocabulary $\times$ Document Length tensor where each word in the document
is one-hot encoded. Though, document lengths vary, so a dataset contains tensors
of different sizes.
...
vector $x \in \reals^D$ to a binary label $y \in \{0, 1\}$ using the perceptron (@Rosenblatt1958).
...
activations,
\begin{align*}
x \xrightarrow{f_1} z^1 \xrightarrow{g_1} h^1 \xrightarrow{f_2} z^2 \xrightarrow{g_2} \hat{y}
\end{align*}
...
1}}$ and $g$ is a nonlinear activation function applied elementwise, like
$\left[g\left(x\right)\right]_{d} =\max(x_{d},0)$. Note that the
argument for $f$ is a generic $v \in \reals^{D_{l}}$. At the first layer, this
is $x$, but at deeper layers, it is the current hidden representation $h^{l -
1}$.
...
\begin{align*}
x \xrightarrow{f_1} z^1 \xrightarrow{g_1} h^1 \xrightarrow{f_2} \dots \xrightarrow{g_{L}} h^{L} \xrightarrow{g_{\text{out}}} \hat{y}
\end{align*}
...
1. There are two types of quantities involved with a deep learning model, and they each play a different role in learning and prediction.
    ...
    - **Activations** are intermediate representations of a single input $x$ as
...
1. Training adjusts $\theta$ so that the final representations $h^{L}$ are
  predictive of $y$.

    ![Distinction between parameters and activations (Credit to Torralba et al.).](figures/params_vs_activations.png){width=70%}

1. If $g$ is differentiable (almost everywhere), the entire network $f = g_{\text{out}} \circ f_{L}
\circ \dots \circ g_{1} \circ f_{1}$ is a composition of nearly smooth
...
improve the loss landscape for stochastic gradient descent.
    ...
    - **Layer Norm**. Like $\ell^2$ normalization, but both centers and scales the $D$ coordinates of a single input,
    \begin{aligned}
    \mu &= \frac{1}{D} \sum_{d=1}^D x_{\text{in}}[d]\\
    \sigma^2 &= \frac{1}{D} \sum_{d=1}^D \left(x_{\text{in}}[d] - \mu\right)^2\\
    x_{\text{out}}[d] &= \gamma_{d} \frac{x_{\text{in}}[d] - \mu}{\sigma} + \beta_{d}
    \end{aligned}
...
understand what a network will do with a new input $x$ by inspecting $\theta$
...
them from data alone is one of the central phenomena associated with deep
...
stored in the tensor `X_grid`, but the block below runs the fitted model on
