...
vecor $x \in \reals^D$ to a binary label $y \in \{0, 1\}$ using the perceptron (@Rosenblatt1958).
    ...
    ![Diagram representing a perceptron. (Credit to Torralba et al.).](figures/perceptron_fig2.png){width=30%}
...
$x$, but at deeper layers, it is the current hidden representation $h^{l - 1}$.
    ...
    _Exercise: Modify this figure to show the shape of the tensor $W_{1}$._
...
```{python}
X_train = torch.tensor(simulation_df[['x1', 'x2']].values, dtype=torch.float32)
