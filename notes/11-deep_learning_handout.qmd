...
be viewed as its own neuron.
  ...
  ![A linear layer can be viewed as a distribution transformer, translating and skewing the inputs according to the parameters $W$ and $b$.](figures/mapping_l2.png)
...
minima of the loss function.
  ...
  \begin{aligned}
    x_{\text{out}}[d] =
    \gamma
    \frac{x_{\text{in}}[d] - \mathbb{E}[x_{\text{in}}[d]]}
         {\sqrt{\text{Var}[x_{\text{in}}[d]]}}
    + \beta
  \end{aligned}
  where $\hat{\mathbb{E}}_{\text{\batch}}$ and
  $\widehat{\text{Var}}_{\text{batch}}$ refer to the empirical mean and variance
  of each coordinate across the current batch.

  The tensor dimension $D_{\text{in}}$ remain unchanged after normalization. The
  free parameters $\gamma \in \reals$ and $\beta \in \text{D_{in}}$ allow each
  neuron to have its mean and variance, which controls the frequency and
  ...
  ![Batch vs. Layer Norm, visualized in the reading.](figures/batchnorm_vs_layernorm_diagram.png)

  -

  ![](figures/mapping_l2.png)
...
representations $h^l$ to a form that can be used to predict $y$.
  ...
  activation to the a probability distribution over $K$ classes. This distribution can be used either for prediction (e.g., most likely class $\to$ image label) or sampling (e.g., sample next word using that distribution)
  \begin{aligned}
      x_{\text{out}}[d] &= \frac{e^{-\tau x_{\text{in}}[d]}}
        {\sum_{k=1}^K e^{-\tau x_{\text{in}}[k]}}
  \end{aligned}
  Higher values of the _temperature_ parameter $\tau$ ``flattens'' the
  distribution across the $K$ classes, which is helpful for sampling more
  diverse outputs.
...
1. The last reason gives a key hint towards explaining AI models. Each layer
...
1. It turns out that, more than simply making the final objective easier, deeper
...
![Deeper layers learn more abstract representations of the input data. Figure from @Bengio2009.](figures/yoshua_bengio_representations.png)
