---
title: "Sparse Autoencoders"
author: Kris Sankaran
date: 2026-04-20
bibliography: references.bib
format:
  html:
    embed-resources: true
    html-math-method: mathjax
    fig-align: center
    code-overflow: wrap
execute:
    echo: true
    message: false
    warning: false
    cache: false
    eval: true
---

::: {style="display: none;"}
$$
\newcommand{\bs}[1]{\mathbf{#1}}
\newcommand{\reals}{\mathbb{R}}
\newcommand{\widebar}[1]{\overline{#1}}
\newcommand{\E}{\mathbb{E}}
\newcommand{\indic}[1]{\mathbb{1}\left\{{#1}\right\}}
\newcommand{\Earg}[1]{\mathbb{E}\left[{#1}\right]}
\newcommand{\Esubarg}[2]{\mathbb{E}_{#1}\left[{#2}\right]}
$$
:::

<style>
.purple { color: #7458d1ff; } /* pastel purple */
.orange { color: #fca020; } /* pastel orange */
.green { color: #3bbe67ff; } /* pastel green */
.darkblue { color: #4a9ceaff; } /* pastel dark blue */
.pink { color: #ee6ec3ff; } /* pastel pink */
</style>

```{r}
#| label: fig-setup
#| echo: false
library(tidyverse)
library(reticulate)
theme_set(theme_classic() + theme(panel.border= element_rect(fill = NA, linewidth = .5)))
set.seed(2026)
```

```{r}
#| label: fig-pysetup
#| echo: false
# includes the SHAP package. Can install it using,
# > conda env create -f stat479_week13.yaml
# where the yaml file is located at: https://github.com/krisrs1128/stat479_notes/blob/master/notes/stat479_week13.yaml
use_condaenv("stat479_week13")
```

_[Readings (up to "Feature Surveys")](https://transformer-circuits.pub/2024/scaling-monosemanticity/) [Code](https://github.com/krisrs1128/stat479_notes/blob/master/notes/15-sparse_autoencoders.qmd)_,  _[Conda Env](https://github.com/krisrs1128/stat479_notes/blob/master/notes/stat479_week13.yaml)_, _[Helpers](https://github.com/krisrs1128/stat479_notes/blob/master/notes/sae_helpers.py)_

Items marked $^{\dagger}$ will not be tested.

## Setup

1. **Goal**. Expand the dimensionality of an LLM's activation space to disentangle learned representations into interpretable components. For any observation's activations $x_n$ (at a given layer), we want the decomposition,
   $$
x_n \approx c + \sum_{\text{few } k} z_{nk}\, v_k
   $$
   where $v_1, \dots, v_K$ are called _atoms_ (dictionary elements) and $z_{nk}
   \geq 0$ are sparse mixing weights.

1. **Requirements.** The method has to scale to realistic LLM models and
datasets, and it shouldn't involve too much manual labeling effort.

1. **Motivation.** Concept activation vectors require curated examples for each
concept -- they are a confirmatory technique. We want an exploratory method that
automatically discovers features. For example, in fairness and safety audiets,
the the most important features aren't known in advance.  Further, since LLMs
are generative, we need do more than explain predictions -- we need to "steer"
model outputs towards (or away from) certain concepts.

1. **Preview.**

   - The technique discovers non-obvious features. One learned atom activates
   strongly on Golden Gate Bridge content, across languages and modalities.

     ![One of the features has high score $z_{nk}$ on samples related to the Golden Gate Bridge.](figures/golden_gate.png){width=80%}

   - Those features can steer generation.
   ![Steering model output using the discovered feature.](figures/golden_gate_steering.png){width=70%}

## Sparse Autoencoder Model

1. **Individual neurons.** Earlier work described individual neurons by finding
their highest-activating examples. E.g., some lower-layer neurons act as edge
detectors.

   ![Neurons in a CNN that activate for particular edge orientations, from @Zeiler2014.](figures/zeiler_edge_detector.png){width=40%}

   However, in higher layers, individual neurons can play multiple roles
   depending on context (the "superposition hypothesis") so single-neuron
   analysis fails.

1. **Dimensionality reduction.** We could apply dimensionality reduction methods
(e.g., PCA or UMAP) to the learned activations and then interpret the resulting
plots.

   ![@Johnson2016 applied dimensionality reduction to a translation model's activations. Each color is one sentence translated into many languages. The model places sentences with the same meaning next to one another regardless of language.](figures/multilingual_tsne.png)

   ![Dimensionality reduction represents $x_n$ using a lower-dimensional $z_n$.](figures/dim_reduction_sae.png){width=45%}

   Unfortunately, this is not so useful at the scale of modern LLMs. The number
   of distinguishable concepts in the training data is much larger than the
   embedding dimension (e.g., the layer in GPT-3 similar to the one studied in
   the reading has 12K dimensions, but the internet contains many more than 12K
   concepts...). We need to _increase_, not reduce, dimensionality.

1. **Notation.** Fix a layer $l$ and set,

   - $x_n \in \mathbb{R}^{D}$: activations of sample $n$ at layer $l$.
   - $v_1, \dots, v_K \in \mathbb{R}^D$: learned atoms, representing the underlying features. Typically $K \gg D$. We will stack these columnwise into $V \in \reals^{D \times K}$.
   - $z_n \in \mathbb{R}^{K}$: sparse, nonnegative weights giving the
   contribution of feature $k$ to sample $n$.

   ![The SAE model represents $x_n$ using a higher-dimensional sparse $z_n$.](figures/expansion_diagram.png){width=45%}

1. **Architecture**. The sparse autoencoder (SAE) has two components^[This is not exactly the standard dictionary learning objective, but the differences are minor (@Yun2021 @Olshausen1996).],

   - Encoder: $\quad z_n = g(Wx_n + b)$, where $g(u) = u \cdot \mathbf{1}[u > 0]$ is the ReLU.
   - Decoder: $\quad \hat{x}_n = Vz_n + c$.

   ![Matrix representation of the encoder.](figures/encoder_matrices.png){width=50%}

   ![Matrix representation of the decoder.](figures/decoder_matrices.png){width=50%}

   Each activation $x_n$ is a sparse, nonnegative mixture of learned atoms $v_k$,
   $$
   x_n \approx \hat{x}_n = c + \sum_{k=1}^{K} z_{nk}\, v_k
   $$
   ($z_{nk}$ is scalar, $v_k \in \mathbb{R}^D$).

   _Exercise: In the reading, what choices of $K$ are considered?_

   _Exercise: Why are the weights $z_{nk}$ nonnegative?_

1. A minimal implementation.

   ```{python}
#| label: sae-class
import torch
import torch.nn as nn
import torch.nn.functional as F

class SAE(nn.Module):
    def __init__(self, D, K):
        super().__init__()
        self.encoder = nn.Linear(D, K)
        self.decoder = nn.Linear(K, D, bias=True)

    def forward(self, x):
        z = F.relu(self.encoder(x))
        x_hat = self.decoder(z)
        return x_hat, z
   ```

1. **Training objective**. The parameters $W \in \mathbb{R}^{K \times D}$, $V \in \mathbb{R}^{D \times K}$, $b \in \mathbb{R}^{K}$, $c \in \mathbb{R}^{D}$ are learned by minimizing

   \begin{align}
   \frac{1}{N}\sum_{n = 1}^{N}\left[\|x_n - \hat{x}_n\|_{2}^{2} + \lambda \sum_{k} z_{nk} \|v_k\|_{2}\right].
   \end{align} {#eq:loss}
   Note that the reconstruction term expands,
   \begin{align}
    \|x_n - \hat{x}_n\|_{2}^{2} = \|x_n - Vz_n - c\|_{2}^{2} = \|x_n - V g\left(W x_n + b\right) - c\|_2^2
   \end{align}
   so depends on $W,V, b$, and $c$.

   ```{python}
#| label: sae_loss
def sae_loss(x, x_hat, z, decoder, lam):
    recon = F.mse_loss(x, x_hat)
    col_norms = decoder.weight.norm(dim=0)  # ||v_k||_2 for each k
    penalty = (z * col_norms).sum(dim=1).mean()
    return recon + lam * penalty
   ```

1. The penalty is essentially an $\ell^{1}$ norm. Since $z_{nk} \geq 0$ and $\|v_k\|_2 \geq 0$, setting $\alpha_k = z_{nk}\|v_k\|_2$ gives
   $$
   \lambda \sum_k z_{nk}\|v_k\|_2 = \lambda \sum_k |\alpha_k|
   $$
   This is a lasso-type penalty on the product of feature length and mixing
   weight. It will encourage most $z_{nk}$ to be exactly zero, so each $\sum_k
   z_{nk} v_k$ involves only a few nonzero terms even though $K$ is large.

1. **Overcomplete basis.** When $K > D$, the dictionary is called overcomplete.
Sparse combinations of its atoms can represent geometric structure that no
$D$-dimensional linear subspace can capture.

   ![Data lying on a low-dimensional subspace](figures/subspace_geometry.png){width=45%}

   ![An overcomplete basis supports more complex data structure.](figures/sae_geometry.png){width=45%}

   _Exercise: Compare and contrast the atoms $v_k$ here with the concept vectors $v_c^l$ from the Concept Activation Vectors notes._

   _Exercise: What is the pattern of zeros in $z_n$ for the samples labeled $A$, $B$, and $C$? What is the relationship between $z_n$ for point $B$ vs. $C$?_

## Synthetic Example

1. We give an example using synthetic data from two ground-truth atoms $v_k$.

   ```{python}
#| label: set-seed
#| echo: false
#| include: false
import numpy as np
np.random.seed(479)
torch.manual_seed(479)
   ```

   ```{python}
#| label: ground-truth
from sae_helpers import generate_v_true, generate_z_true

N = 500
v_true = generate_v_true()
K_true = v_true.shape[1]
z_true = generate_z_true(K_true=K_true, N=N)
X = v_true @ z_true + np.random.randn(2, N) * 0.05  # 2 x N
   ```

   We deliberately misspecify $K$ (3 instead of 2) to test robustness.

   ```{python}
#| label: sae-fit
D, K = 2, 3
model = SAE(D, K)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
X_tensor = torch.tensor(X.T, dtype=torch.float32)  # N x 2

lam = 0.5
for epoch in range(1000):
    x_hat, z = model(X_tensor)
    loss = sae_loss(X_tensor, x_hat, z, model.decoder, lam)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
   ```

   We extract the learned atoms and compare with ground truth.

   ```{python}
#| label: learned-features
V_learned = model.decoder.weight.detach().numpy()  # D x K
   ```

   ```{python}
#| label: plot-results
#| echo: false
from sae_helpers import plot_dictionary_recovery

plot_dictionary_recovery(X, v_true, V_learned, K_true=K_true)
   ```

   _Exercise: How would you compare the true and learned $z_{nk}$?_

## Scaling Laws

1. Training SAE's with large $K$ and across large dataset is computationally
expensive. Holding all other training hyperparameters constant, compute cost $C$
increases proportionally to $K$ and the number of gradient descent training
steps $T$, $C = TK$.

  * For a fixed $C$, how should we allocate budget across $T$ and $K$?
  * For a proposed increase in $C$, by how much would we expect the training loss @eq-loss to decrease?

   By sweeping values of $T$ and $K$ for gradually increase compute budgets, we
   can answer these questions in a way that allows us to plan large $C$ training
   runs. In the LLM literature, this type of experimental design is called a
   scaling law analysis.

1. Consider increasing budgets $C_1 < \dots < C_M$. For each $m$, evaluate
$\{\left(T_m^{l}, K_m^{l}\right)\}_{l = 1}^{L}$ pairs satisfying $C_m = T_m^l
K_m^l$ (sometimes called the IsoFLOP contour). At this $m$, one choice
$T_{m}^{l^*}, K_{m}^{l^\ast}$ minimizes the loss @eq-loss. For notational
convenience, denote this by $T^\ast\left(C_m\right)$, and similarly define
$K^{\ast}\left(C_m\right)$.

1. Unfortunately, the reading doesn't annotate the axes of the scaling law
analysis. But we can see how the loss varies across $C_m$ (separate curves) and
$l$ (points along a single curve).

   ![Loss against compute budget in the scaling law experiments.](figures/sae_scaling_law.png)

1. We can further relate the compute budget to the optimal choice $T^\ast$ and
$K^\ast$. It turns out that these can be modeled well using a power law curve
(i.e., linear on a log scale),

  $$
T^\ast\left(C_m\right) \approx \beta_{0} + \beta_1 C_m \\
K^\ast\left(C_m\right) \approx \alpha_0 + \alpha_1 C_m
  $$

   Given a new target compute budget $C$, we fit the SAE using
   $\hat{T}\left(C\right) = \hat{\beta}_0 + \beta_{1}C$ training steps and
   $\hat{K}\left(C\right) = \hat{\alpha}_{0} + \hat{\alpha}_{1}C$ features.
   Note that this approach doesn't guarantee that $C =
   \hat{T}\left(C)\hat{K}\left(C\right)$. For a way to enforce that constraint,
   see Approach 3 in @hoffman2024.

   ![Power law relationships for training steps and feature count in the SAE experiments.](figures/scaling_law_allocation.png)

## Autointerpretability

1. It's not possible to manually annotate millions features, so we have to rely
on automatic interpretability methods instead. The basic procedure is to train
and evaluate an intermediate explainer model.

1. **Explainer training**.

   - For a feature $k$, randomly sample indices $n$ across quantiles of $z_{nk}
   | z_{nk} > 0$. Call the stratified sampling result $D^{\text{explain}}$ and
   gather the associated source texts .
   - Show the source texts and $z_{nk}$ to an explainer LLM and prompt it to
   generate a natural language description $d_{k}$ to summarize feature $k$.

1. The reading is vague about what prompts they actually use. But related
peer-reveiwed papers give details. For example @paulo prompt the explainer LLM
using,

   ![](figures/paulo_explainer_prompt.png)

   Similarly @simon2024 use the prompt below to automatically generate
   descriptions for SAE features from a protein language model,

   ![](figures/simon_explainer_prompt.png)

1. **Explainer evaluation**. Gather new samples $D^{\text{eval}}$. Show the
model the corresponding texts and the descriptions $d_{k}$, and ask it to
predict $z_{nk}$. The quality of the autointerpretations $d_{k}$ is summarized
as $\text{Cor}\left(z_{nk}, \hat{z}_{nk}\right)$ for $n \in D^{\text{eval}}$.

For example, the evaluation prompts for the two papers mentioned above are,

  ![]()  ![]()

## Sensitivity, Specificity, and Control

1. The SAE gives us features $k$ and the autointerpretability pipeline gives
human-understandable $d_k$. We don't know the quality of the learned features
though.

  - Sensitivity: What
  - Specificity: What


## Code Example
