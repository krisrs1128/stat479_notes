...
1. _Model validation_. Therefore, saliency maps can be used as a form of model
validation. If the map highlights the right object, the model is using the right
information. For example, @ref found ___. But as @adebayo2018 caution, this
logic is not sufficient without additional checks. Some saliency methods
highlight visually salient regions regardless of what the model has actually
learned.
...
the classification. Note that $\nabla_{x}f_{k, \theta}$ is a gradient with
...
\theta}\left(x^\ast\right).
$$
This suppresses attributions in the darker regions where $x_j^\ast$ \approx 0$.

1. *Integrated Gradients*. The gradients of the softmax function vanish whwen the activations are large. Therefore, a confident model will have $\nabla_x f_{\theta, k}$ near zero even on important pixels. Integrated gradients address this by averaging over a path from a baseline $x_0$ (e.g., a black, all zeros image) to $x^\ast$,
$$
...
setups that deliberately have no signal and which serves as a reference point
for interpretation.
...
corrupts the data. Specifically, it permutes training labels $y_i$ adn retrains
...
associations, so any explanation that is sensitive to the true $x \text{vs. } y$
...
which is initialized on the trained model and and applied to samples $x^*$.
...
1. `IntegratedGradients` highlights the bird moer clearly, so we must be in a
