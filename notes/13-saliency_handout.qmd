...
considers $\left|E\left(x^*, f\right)\right|$) suggests that at least some of
...
$E_{\text{grad}}\left(x^*, f\right)$ is implemented by the `Saliency` class,
...
1. Nearly the same interface works with $E_{\text{IG}}\left(x^*, f\right)$,
