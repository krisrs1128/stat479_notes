...
1. **Preview**. Each block $f_{\theta_{m}}$ will have the form,
  ...
  ![](figures/transformer_summary.png){width=60%}
  ...
  Everything below the first $+$ is Stage 1 and everything above is Stage 2.
...
Each layer enriches token representations by sharing information across tokens
(Stage 1) and features (Stage 2). Stacking layers allows the representations to
be mixed again so that higher-level, indirect relationships can emerge.
...
A = outputs.attentions[10][0, 11].detach()  # layer 11, batch 0, head 12
```
